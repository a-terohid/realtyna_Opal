...
type Props = {
  params: { state: string; filter: string }
...
async function getData({ params, searchParams }: Props) {
  ...
  const stateSubRoute = statePageSubRoutes[params.filter]

  if (!stateSubRoute) {
    notFound()
  }

  const { filter } = stateSubRoute({})
  searchParams = {
    ...searchParams,
    ...filter
  }
  ...
  const { title, description, isRemoveNumbers } = stateSubRoute({
    sitename: siteName,
    state: params.state,
