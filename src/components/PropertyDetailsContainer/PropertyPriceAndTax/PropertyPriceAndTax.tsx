import React from "react"

import type { ListingsValue } from "@/types/listings.type"

import DetailRow from "../PropertyBasicDetails/DetailRow/DetailRow"

interface IProps {
  details: ListingsValue
}

const PropertyPriceAndTax: React.FC<IProps> = ({ details }) => {
  const data = [
    {
      key: 1,
      value: details.TaxYear || details.TaxAnnualAmount,
      title: "Tax Year",
      isMulti: false
    },

    {
      key: 2,
      value: details.MFR_CalculatedListPriceByCalculatedSqFt,
      title: "Price Per S qft",
      isMulti: false
    },
    {
      key: 3,
      value: details.CommunityFeatures,
      title: "Association Amenities",
      isMulti: true
    },

    {
      key: 4,
      value: details.ParcelNumber,
      title: "Parcel Number",
      isMulti: false
    },
    {
      key: 5,
      value: details.AssociationFee,
      title: "Association Fee",
      isMulti: false
    },
    {
      key: 6,
      value: details.AssociationFeeIncludes,
      title: "Association Fee Includes",
      isMulti: true
    }
  ]

  const newData = data.filter((val) => {
    if (val.value && val.value !== "NONE") {
      return val
    }
  })

  return (
    <>
      {newData?.length > 0 ? (
        <section className="col-span-full flex flex-col items-start justify-center gap-[.875rem]">
          <h2 className="text-2xl font-bold text-black">
            Property Price and Tax
          </h2>
          <div className="grid w-full grid-cols-1 gap-[25px] rounded-[.9375rem] p-[.9375rem] shadow-primary md:grid-cols-2">
            {newData.map((item) => {
              return (
                <DetailRow
                  value={item.value}
                  key={item.key}
                  label={item.title}
                  isMulti={item.isMulti}
                />
              )
            })}
          </div>
        </section>
      ) : null}
    </>
  )
}

export default PropertyPriceAndTax
